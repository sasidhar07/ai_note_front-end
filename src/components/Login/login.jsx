...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://ai-notes-backend-0c9d.onrender.com/api/auth/${endpoint}`, {
        method: 'POST',
    ...
    <div className="login-page">
      <div className="form-container">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {!isLogin && <input type="password" id="confirmPassword" placeholder="Confirm Password" value={credentials.confirmPassword} onChange={handleChange} />}
          <button type="submit" disabled={isLoading}>{isLogin ? 'Login' : 'Sign Up'}</button>
        </form>
        <p onClick={toggleForm}>{isLogin ? 'Create an account' : 'Already have an account? Login'}</p>
      </div>
    </div>
  );
};
